...
size is in `Arlib.KnowledgeCompilation`.  `Pinned` narrows this — it re-states
the clauses with `c` and `d` as parameters, so a development that has computed an
exponent can say which one — but a pinned bound is still an upper bound on a
recorded number, and the gap is not closed.
