...
| `DepSampling` | **`IsFPAUS` with the solution type depending on the instance**, and the compiled reason the generalisation was needed. `IsFPAUS size g A` fixes one `Ω` before the instance is named, so a *family* whose solution objects are typed by the instance — conjunctive queries over a growing constant domain, answers in `List (C m)`; tree automata over a growing alphabet, accepted trees in `LTree (Γ m)` — is not an `IsFPAUS` instance family at all. `IsFPAUSDep` takes `Ω : α → Type v` and repeats the *same three clauses fibrewise*, which is possible only because every clause of `IsFPAUS` already quantifies over `w` before mentioning `Ω`: the fixed `Ω` was convenience, not content. **`isFPAUS_iff_dep` is an `Iff`**, not a one-way implication — the constant family `Ω := fun _ => Ω₀` gives back `IsFPAUS` on the nose, which is what makes this a generalisation rather than a neighbouring notion and what leaves every existing consumer's theorem meaning exactly what it meant. `isFPAUSDep_iff_tagged` settles the workaround: for **any** family of embeddings `tag w : Ω w ↪ T` into a single type, with nothing assumed relating the `tag w` to each other, the tagged `IsFPAUS` statement and the untagged dependent one are *equivalent* — so tagging is content-free, and in particular a tagged sampler is **not** additionally required to be uniform across fibres, since `uniform` compares `Pr[output = x]` only with `|g w|` at the single `w` that produced `x`. The generality is load-bearing: `CQCount.Capstone.KAtoms` tags by a `Σ`-index coarser than the instance, which `sigmaEmb`'s special case would not cover. `comp_bijection` transfers along a bijective reduction with **both** solution types moving, `comp_bijection_pinned_on` / `comp_bijection_pinned` carry the exponent, and `PinnedTime` / `Charges` port verbatim with `pinnedTime_of_cost_zero` and `Charges.map_add` — the sandwich is not repaired by dependent types and the module says so. **Nothing analogous is built for `IsFPRAS`, and that is a finding, not an omission**: an FPRAS returns a *number*, `ℝ` is written into the definition rather than being a parameter, and `isFPRAS_no_dependent_output` is the statement that adding a dependent solution-set datum changes the proposition not at all; a family of counting problems whose solution objects move is already an `IsFPRAS` over a `Σ`-type with no new definition and no tagging. Also **not** ported: `Sampling`'s `PreprocessedSampler` assembly and `Parsimonious`/`Pinned`'s bundled reductions — the former because every one of its inputs is a statement about a *single* `PMF` on a *single* type and so already applies in a fibre, the latter because they marry a universe-blind counting datum to a sampling one and a caller whose types move takes the two transfer theorems separately. The rule the module states: a lemma quantifying over instances needs the dependent form; a lemma about one run does not. `outProb_map'`, `outProbR_map'` and `mem_support_map'` are `Counting`'s lemmas with their source and target universes decoupled, added beside the originals so that no elaborated consumer changes. `exists_isFPAUSDep` is the dependent twin of `CQCount.Capstone.exists_isFPAUS` — the *exactly* uniform zero-cost sampler `trivSamplerDep`, defined in each fibre's own type, is an `IsFPAUSDep` for **every** dependent solution family whatever — so it is simultaneously a satisfiability certificate and the statement that an `IsFPAUSDep` hypothesis on a reduction's target is **free** in this cost model, being precisely the object `pinnedTime_of_cost_zero` describes and `Charges` excludes. The non-degenerate witness — a family whose constant domain genuinely grows and whose solution set has `m+2` elements at member `m` — is `CQCount.Capstone.KDomain.GrowC`, since it needs a source of real instances. |
...
import Arlib.Approximation.DepSampling
