...
Göös–Kiefer–Yuan's bonus result (`thm: error`, §6,
`source/kc/goos/parts/applications.tex:2`) says that approximate nonnegative
...
it at §4's `F` with no further work, and so that the two halves of `thm: error`
...
generic facts about `HasNNRankLE` that `Communication/TwoParty.lean` does not
...
`Communication/TwoParty.lean` documents, met here for the first time in this
area.
...
The two halves of `thm: error` (`source/kc/goos/parts/introduction.tex:61`,
`source/kc/goos/parts/applications.tex:25`) are about the *same* boolean matrix
...
* at `10⁻⁵`, `rk⁺` is huge — `2^{Ω̃(m²)}`, which is `thm: hard-or`, §4's
  machinery (`source/kc/goos/parts/union.tex`).

The upper half is proved here.  The lower half is **not**, and cannot be
assembled from what this repository has: §4's hardness is delivered as a lower
bound on the approximate nonnegative *degree* of the one-party `f^∨`
(`Communication/ConicalJunta.lean`) and on `Par₁` in the variable-partition
model, never lifted to an approximate nonnegative *rank* statement about the
two-party `F^∨` at error `10⁻⁵`.  So, following `ROADMAP.md` §1.2 and the idiom
of `Imported.lean`, it enters as an imported, **inhabited** bundle `ErrorHard`,
and `thm_error` assembles the two halves into the gap the paper states. -/
...
/-- **§4's hard two-party function, with its `10⁻⁵` lower bound**
[IMPORTED — Göös–Kiefer–Yuan `thm: hard-or`, proved in
`source/kc/goos/parts/union.tex` and quoted at
`source/kc/goos/parts/applications.tex:25`].
...
* `lowerBound` bounds `rk⁺_{10⁻⁵}(F^∨)` from below — the paper's
  `anrank_{10⁻⁵}(F^∨) ≥ 2^{Ω̃(m²)}`, the content of §4.

Both are carried as explicit numbers rather than `2^m` / `2^{Ω̃(m²)}`, per
`ROADMAP.md` §5.  The `hard` field is what is genuinely imported; `nnRankLE` is
...
structure ErrorHard (X Y : Type*) (rankBound lowerBound : ℕ) where
  ...
  /-- `rk⁺_{10⁻⁵}(F^∨) ≥ lowerBound`: §4's `2^{Ω̃(m²)}` lower bound at error
  `10⁻⁵`. -/
  hard : lowerBound ≤ anRank (1 / 10 ^ 5) (tpIndicator (orExtend F))

/-- **`thm: error`, both halves at the concrete hard function**
(`source/kc/goos/parts/introduction.tex:61`,
`source/kc/goos/parts/applications.tex:25`).

For `M := F^∨` the matrix of §4's hard function, the `1/4`-approximate
