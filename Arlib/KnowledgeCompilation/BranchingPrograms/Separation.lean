...
and CNFs of bounded treewidth* (`source/kc/razgon/FBDDJOURN.tex`) meet here.

`BranchingPrograms/NROBP.lean` proves Theorem `nrobplbdmw`
(`source/kc/razgon/FBDDJOURN.tex:515`) *conditionally*: it reduces the size of a
...
(`source/kc/razgon/FBDDJOURN.tex:651`), which is exactly that bound.  This file
...
exponent to be an integer.  `nrobplbdmw` counts *nodes*, so its bound is a
...
(`source/kc/razgon/FBDDJOURN.tex:1208`) reduces the general case to the uniform
...
/-- **Theorem `nrobplbdmw`** (`source/kc/razgon/FBDDJOURN.tex:515`),
...
This is `NROBP.nrobplbdmw` with its `hEngine` hypothesis discharged by
...
theorem two_rpow_le_size {G : SimpleGraph V} [DecidableRel G.Adj] {t x size : ℕ}
  ...
  have hbound : ⌈(2 : ℝ) ^ ((t : ℝ) / TCover.f x)⌉₊ ≤ size :=
    NROBP.nrobplbdmw Z hro hu hR hmw fun A hsize hcov =>
...
`source/kc/razgon/FBDDJOURN.tex:476`. -/
...
/-- **Theorem `maintheor`** (`source/kc/razgon/FBDDJOURN.tex:476`): for the
...
`Arlib/KnowledgeCompilation/ROADMAP.md` §5 deliberately does not do: the paper's
...
.binTree_pathGraph_bounds` carries the vertex count and the treewidth bound
alongside, so a reader can perform the substitution and see exactly what it
costs. -/
theorem maintheor {p r size : ℕ} (hr : Nat.clog 2 p ≤ r)
  ...
  have hb := TreeProduct.binTree_pathGraph_bounds p r hr
  two_rpow_le_size Z hro hu hR hb.2.2.2 (maxDegree_le_of_maxDegreeLe hb.2.1)
