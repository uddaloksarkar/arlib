...
(§4, `source/kc/darwiche/draft.tex:264`).  This file formalizes the paper's main
theoretical separation: there are classes `f_n` and `g_n(·, Z)` with `f_n` emf to
`g_n`, such that **every** d-DNNF for `f_n` is exponential while **some** d-DNNF
for `g_n` is polynomial.  Forgetting the single auxiliary variable `Z` from the
compact d-DNNF for `g_n` yields a compact (non-deterministic) DNNF for `f_n`,
which no d-DNNF can match.

## The concrete functions (`:294`)
...
* `row_n(M) = ⊕ᵢ h_n(Rᵢ)`, the XOR over rows (`rowFn`); `col_n` over columns
  (`colFn`);
* `f_n(M) = row_n(M) ∨ col_n(M)` — the **Sauerhoff function** (`sauerhoff`);
...
The **heart of this file, proved unconditionally, is `emf_sauerhoff_gFun`**:
`f_n ≡ ∃Z. g_n`.  The proof is entirely structural and turns on one fact — that
`row_n` and `col_n` read only the matrix variables, never `Z`.  Given an
assignment `α`, its extension with `Z := true` makes `g_n` equal to `row_n(α)`
and with `Z := false` equal to `col_n(α)`; so `∃Z. g_n` fires iff
`row_n(α) ∨ col_n(α) = f_n(α)`.  Since `emf` in `Forgetting.Basic` is *definitionally*
`f(α) ↔ ∃β≡α off Y. g(β)`, this is the whole content, and by
`emf_iff_eq_forgetFun` it upgrades to the equation `f_n = forgetFun {Z} g_n`
(`sauerhoff_eq_forgetFun`).  The internal shape of `h_n` — divisibility by 3,
parity of the XOR — is *irrelevant* to this proof, and is never unfolded; it is
defined faithfully only so that the classes are the paper's.
...
`LowerBounds/Imported.lean` (`ROADMAP.md` §1.3), never as `axiom`s:
...
`cor_forgetting` packages the separation as the paper's corollary: d-DNNF (and
FBDD) do not support polynomial-time single-variable forgetting.  It exhibits the
compact d-DNNF for `g_n` whose `{Z}`-forgetting computes `f_n`, together with the
...
`Sum.inr ()` (§4, `source/kc/darwiche/draft.tex:294`). -/
...
def mentry {n : ℕ} (α : SepVar n → Bool) (i j : Fin n) : Bool := α (Sum.inl (i, j))
...
/-- **`h_n`** (§4, `:296`): `true` iff the number of `1`s among the inputs is
...
`true`.  This is `⊕ᵢ` at `:304`, written as the parity of a cardinality so that no
...
def rowBit {n : ℕ} (α : SepVar n → Bool) (i : Fin n) : Bool :=
  divBy3 (fun j => mentry α i j)

/-- The `h_n`-value of column `i` of the matrix, `h_n(Cᵢ)`. -/
def colBit {n : ℕ} (α : SepVar n → Bool) (i : Fin n) : Bool :=
  divBy3 (fun j => mentry α j i)

/-- **`row_n(M) = ⊕ᵢ h_n(Rᵢ)`** (§4, `:304`). -/
def rowFn {n : ℕ} (α : SepVar n → Bool) : Bool := xorFam (rowBit α)

/-- **`col_n(M) = ⊕ᵢ h_n(Cᵢ)`** (§4, `:305`). -/
def colFn {n : ℕ} (α : SepVar n → Bool) : Bool := xorFam (colBit α)

/-- **The Sauerhoff function `f_n(M) = row_n(M) ∨ col_n(M)`** (§4, `:311`). -/
def sauerhoff (n : ℕ) (α : SepVar n → Bool) : Bool := rowFn α || colFn α

/-- **`g_n(M, Z) = (Z ∧ row_n(M)) ∨ (¬Z ∧ col_n(M))`** (§4, `:300`). -/
...
— is the whole engine of the emf proof. -/

/-- If two assignments agree on every matrix variable, they agree on `mentry`. -/
lemma mentry_congr {n : ℕ} {α β : SepVar n → Bool}
    (h : ∀ i j, α (Sum.inl (i, j)) = β (Sum.inl (i, j))) (i j : Fin n) :
    mentry α i j = mentry β i j := h i j
...
lemma rowFn_congr {n : ℕ} {α β : SepVar n → Bool}
  ...
  have : rowBit α = rowBit β := by
    funext i; unfold rowBit; congr 1; funext j; exact mentry_congr h i j
  rw [rowFn, rowFn, this]
...
lemma colFn_congr {n : ℕ} {α β : SepVar n → Bool}
  ...
  have : colBit α = colBit β := by
    funext i; unfold colBit; congr 1; funext j; exact mentry_congr h j i
  rw [colFn, colFn, this]
...
/-! ## The emf equivalence — the provable heart of the file -/

/-- **`f_n` is equivalent modulo forgetting `Z` to `g_n`** (§4, `:314`):
`f_n(M) ≡ ∃Z. g_n(M, Z)`.

The two directions of `emf` (definition in `Forgetting.Basic`, `:529`):
...
* Conversely any `β` agreeing with `α` off `{Z}` has `row_n(β) = row_n(α)` and
  ...
  hence `f_n(α)`. -/
theorem emf_sauerhoff_gFun (n : ℕ) : emf {sepZ n} (sauerhoff n) (gFun n) := by
  intro α
  constructor
  · intro hf
    rw [sauerhoff, Bool.or_eq_true] at hf
  ...
  · rintro ⟨β, hβ, hg⟩
    ...
    rw [sauerhoff, Bool.or_eq_true]
    rcases hg with h | h
    · rw [Bool.and_eq_true] at h; exact Or.inl h.2
    · rw [Bool.and_eq_true] at h; exact Or.inr h.2

/-- **The emf equivalence as an equation** (via `emf_iff_eq_forgetFun`, `:556`):
...
theorem sauerhoff_eq_forgetFun (n : ℕ) :
    sauerhoff n = forgetFun {sepZ n} (gFun n) :=
  emf_iff_eq_forgetFun.mp (emf_sauerhoff_gFun n)
...
numeric bound, in the style of `LowerBounds/Imported.lean` (`ROADMAP.md` §1.3), so
that `thm_sep` visibly rests on exactly these two hypotheses and on nothing else.
Both are inhabited below, so the conditional is not vacuous. -/

/-- **Clause (ii) — the d-DNNF lower bound for the Sauerhoff function**
(Bova–Capelli–Mengel–Slivovsky, imported via §4, `source/kc/darwiche/draft.tex:317`).

Every d-DNNF computing `f_n` has at least `bound` nodes; the imported theorem
supplies a `bound` exponential in `n`.  Following `ROADMAP.md` §5 the bound is an
...
structure SauerhoffdDNNFLowerBound (n : ℕ) where
  ...
  hard : ∀ C : NNF (SepVar n), C.IsdDNNF → C.Computes (sauerhoff n) → bound ≤ C.size

/-- **Clause (iii) — the polynomial d-DNNF for `g_n`** (§4, `:319`: `row_n` and
...
/-- **`thm:sep`** (§4, `source/kc/darwiche/draft.tex:264`).

Conditional on the two imported bundles, the three clauses of the paper's theorem
hold simultaneously for the concrete classes `f_n = sauerhoff n` and `g_n = gFun n`:

* **(i)** `f_n` is emf to `g_n` — proved unconditionally, `emf_sauerhoff_gFun`;
...
recovers `f_n`. -/
theorem thm_sep (n : ℕ) (lb : SauerhoffdDNNFLowerBound n) (ub : GndDNNFUpperBound n) :
    emf {sepZ n} (sauerhoff n) (gFun n) ∧
      (∀ C : NNF (SepVar n), C.IsdDNNF → C.Computes (sauerhoff n) → lb.bound ≤ C.size) ∧
      (∃ C : NNF (SepVar n), C.IsdDNNF ∧ C.Computes (gFun n) ∧ C.size ≤ ub.bound) :=
  ⟨emf_sauerhoff_gFun n, lb.hard, ub.witness⟩

/-- **The corollary** (§4, `:330`): d-DNNF (and FBDD) do not support polynomial-time
...
(`sauerhoff_eq_forgetFun`), yet **every** d-DNNF for that forgotten function has at
...
theorem cor_forgetting (n : ℕ) (lb : SauerhoffdDNNFLowerBound n) (ub : GndDNNFUpperBound n) :
  ...
  apply lb.hard D hDd
  intro α
  rw [hDc α, hCeval, ← sauerhoff_eq_forgetFun]
